...
export class DocumentTitleReplacer {
    public replaceDocumentTitle(
        ...
        if (htmlDocument.documentElement.lang === 'zh-TW')
            return onTraditinalChinesePage(htmlDocument, genreWordConverter);
    }
}
...
function onTraditinalChinesePage(
    htmlDocument      : Document,
    genreWordConverter: GenreWordConverter
): void {
    /*
        日本語版と同様の形式であるようなので、そのまま丸コピ
    */
    const matches = htmlDocument.title.match('「\(.+\)」');
    const matched = (matches !== null);
